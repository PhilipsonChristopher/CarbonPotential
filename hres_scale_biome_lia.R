...
## resample to hansen
writeRaster(soil_crop, paste0(wd,"/tmp/tile",tile,"/soil_crop",tile,".tif"),overwrite=TRUE)
myfile=paste0(wd,"/tmp/tile",tile,"/soil_crop",tile,".tif")
...
## resample to hansen
writeRaster(vlia_crop, paste0(wd,"/tmp/tile",tile,"/vlia_crop",tile,".tif"), overwrite=TRUE)
myfile=paste0(wd,"/tmp/tile",tile,"/vlia_crop",tile,".tif")
if(!file.exists(paste0( myfile,"RESAMP",tile,".tif"))){
	R2 = hansen
	Routput=paste0(myfile,"RESAMP",tile,".tif")
	t1 <- c(xmin(R2), ymin(R2), xmax(R2), ymax(R2))
...
# mask hansen by vlia 
if(!file.exists(paste0(wd,"/tmp/tile",tile,"/hansen_mask",tile,".tif"))){
	hansen_vlia =hansen*vlia30
	writeRaster(hansen_vlia,paste0(wd,"/tmp/tile",tile,"/hansen_mask",tile,".tif"),overwrite=T)
	}
## mask AGB by vlia 
if(!file.exists(paste0(wd,"/tmp/tile",tile,"/AGB_mask",tile,".tif"))){
	AGB_vlia =AGB*vlia30
	writeRaster(AGB_vlia,paste0(wd,"/tmp/tile",tile,"/AGB_mask",tile,".tif"),overwrite=T)
	}
## mask soc by vlia 
if(!file.exists(paste0(wd,"/tmp/tile",tile,"/hengl30_mask",tile,".tif"))){
	hengl30_vlia =hengl30*vlia30
	writeRaster(hengl30_vlia,paste0(wd,"/tmp/tile",tile,"/hengl30_mask",tile,".tif"),overwrite=T)
	}
# ==============================================================================
...
dir.create(outDir)
hansen_vlia=raster(paste0(wd,"/tmp/tile",tile,"/hansen_mask",tile,".tif"))
AGB_vlia=raster(paste0(wd,"/tmp/tile",tile,"/AGB_mask",tile,".tif"))
hengl30_vlia=raster(paste0(wd,"/tmp/tile",tile,"/hengl30_mask",tile,".tif"))
...
MYBIOME=as.vector(biome@data[biome@data<15]) #all ecoregs in grid
