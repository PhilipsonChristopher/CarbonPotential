...
### 1.
(1) retrieve all global data tiles (AGB, SOC, CANOPY), AGB tiles define the computation domain. We loop the algorithm over all tiles that constitute the entire global land surface (2) resample SOC and LIA tiles to a common 30m grid (3) mask all data layers by LIA to extract only low impact regions for further analysis.(4) extract a 10000 random-point sample of values of each component (SOC, AGB and CANOPY) for each member of ECO unit (biomes or ecoregions). This results in a number of results tables per tile (AGB, SOC, CANOPY, 3 cols x 10000 rows) equal to number of biome or ecoregion members within a tile. (5) Finally, we aggregate all values by biome/ecoregion member resulting in global datasets per member of SOC, AGB, CANOPY.
